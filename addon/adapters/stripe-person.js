...
export default class StripePersonAdapter extends ApplicationAdapter {
  ...
  urlForFindRecord(id, modelName, snapshot) {
    return `${this.urlFromSnapshot(snapshot)}/${this.pathForType(
      modelName
    )}/${id}`;
  }
  ...
  urlFromSnapshot(snapshot) {
    ...
    assert(
      'You must pass an stripe-account model as <account> in the adapter options',
      isPresent(account)
    );
