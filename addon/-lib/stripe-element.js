...
export default class StripeElementComponent extends Component {
  ...
  constructor() {
    ...
    this._elementHandlers = BUILTIN_EVENT_HANDLERS.map((eventHandler) =>
      Object.assign({}, eventHandler, {
        handler: this[eventHandler.method].bind(this),
      })
    );
  }
  ...
  async createElement(type, options) {
    ...
    this._elementHandlers.forEach((elementHandler) =>
      this._element.on(elementHandler.event, elementHandler.handler)
    );
  ...
  async createToken(element, [create]) {
    if (create) {
      ...
      (get(this, 'args.token.callback') === undefined
        ? noOp
        : get(this, 'args.token.callback'))(model);
    }
  }
  ...
  async createPaymentMethod(element, [create]) {
    if (create) {
      ...
      const paymentMethod = await this._createPaymentMethod(
      ...
      // Notify the parent that we have created a token from the model.
      (get(this, 'args.paymentMethod.callback') === undefined
        ? noOp
        : get(this, 'args.paymentMethod.callback'))(model);
    }
  }
  ...
  serializePaymentMethod(store, paymentMethod) {
    ...
    return serializer.normalizeSaveResponse(
      this.store,
      modelClass,
      paymentMethod
    );
  ...
  async _createPaymentMethod(type, options = {}) {
    return this.stripe.createPaymentMethod(
      Object.assign({}, options, { type, card: this._element })
    );
  }
